...
static void MD5_memcpy(unsigned char* output, const unsigned char* input, size_t len);
static void MD5_memset(unsigned char* output, int value, size_t len);
...
// === Обновление (добавление данных, делятся входные данные на блоки по 512 бит) === //
...
// === Преобразование блока (ядро, где 64 шага, 4 раунда * 16 операций для функций F,G,H,I) === //
/* Шаг: перемешивание A,B,C,D; добавление констант, сдвиги и суммирование в 32-битной арифметике */
...
// === Утилиты для конвертации uint32 и байты === //
static void Encode(unsigned char* output, const uint32_t* input, size_t len) {
...
char* MD5File(const char* filename) {
	...
	if (!file) return _strdup("Ошибка открытия файла");
